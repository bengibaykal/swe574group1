...
def PopularItems_User(request):
    communities = Community.objects.filter(joined_users=request.user)
    posts = Post.objects.all()

    # The Most Popular Community, Post Template and Posts
    popular_communities = Community.objects.annotate(number_of_posts=Count("post")).order_by("-number_of_posts")[:1]
    popular_post_templates = PostTemplate.objects.annotate(number_of_posts=Count("post_template")).order_by(
        "-number_of_posts")[:1]
    popular_posts = Post.objects.annotate(number_of_comments=Count("related_post")).order_by("-number_of_comments")[:1]
    ...
    the_most_creative_users_completed = sorted(the_most_creative_users, key=lambda x: x[1], reverse=True)[:20]
    ...
    return render(request, 'user/popular_items_user.html',
                  context={
                      'ctype': ContentType.objects.get_for_model(USER_MODEL),
                      'communities': communities,
                      'popular_communities': popular_communities,
...
def PopularItems_Community(request):
    ...
    # The Most Popular Community, Post Template and Posts
    popular_communities = Community.objects.annotate(number_of_posts=Count("post")).order_by("-number_of_posts")[:20]
    popular_post_templates = PostTemplate.objects.annotate(number_of_posts=Count("post_template")).order_by(
        "-number_of_posts")[:1]
    popular_posts = Post.objects.annotate(number_of_comments=Count("related_post")).order_by("-number_of_comments")[:1]
    ...
    the_most_creative_users_completed = sorted(the_most_creative_users, key=lambda x: x[1], reverse=True)[:1]
    ...
    return render(request, 'user/popular_items_community.html',
                  context={
                      'ctype': ContentType.objects.get_for_model(USER_MODEL),
                      'communities': communities,
                      'popular_communities': popular_communities,
...
def PopularItems_PostTemplate(request):
    communities = Community.objects.filter(joined_users=request.user)
    posts = Post.objects.all()
    ...
    return render(request, 'user/popular_items_post_template.html',
                  context={
                      'ctype': ContentType.objects.get_for_model(USER_MODEL),
                      'communities': communities,
                      'popular_communities': popular_communities,
...
def PopularItems_Post(request):
    ...
    popular_post_templates = PostTemplate.objects.annotate(number_of_posts=Count("post_template")).order_by(
        "-number_of_posts")[:1]
    popular_posts = Post.objects.annotate(number_of_comments=Count("related_post")).order_by("-number_of_comments")[:20]
    ...
    return render(request, 'user/popular_items_post.html',
                  context={
                      'ctype': ContentType.objects.get_for_model(USER_MODEL),
                      'communities': communities,
                      'popular_communities': popular_communities,
